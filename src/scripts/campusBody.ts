export function initCampusBody() {
	const body = document.querySelector('.view-body-campus');
	if (!body) return;
	...
	const steps = Array.from(body.querySelectorAll<HTMLElement>('.cb-step'));
	if (steps.length) {
	...
	if (isCampus()) setTimeout(run, 400);
	window.addEventListener('swizel:viewchange', (e) => {
		if ((e as CustomEvent).detail === 'campus') {
			played = false;
			setTimeout(run, 300);
		}
	});
